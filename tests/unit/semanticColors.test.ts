...
describe('semantic color tokens', () => {
  ...
  it('maps key UI signals to semantic colors', () => {
    expect(selectorBlock('.agent-toggle.on')).toContain('background: rgba(var(--color-accent-rgb), 0.22);')
    ...
    expect(selectorBlock('.composer-mode-option.active.agent')).toContain('color: var(--color-accent);')
